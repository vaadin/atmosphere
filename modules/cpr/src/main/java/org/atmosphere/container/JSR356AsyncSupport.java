...
public class JSR356AsyncSupport extends Servlet30CometSupport {
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(JSR356AsyncSupport.class);
    ...
    public JSR356AsyncSupport(AtmosphereConfig config, ServletContext ctx) {
        super(config);
        ServerContainer container = (ServerContainer) ctx
        ...
        int pathLength = 5;
        String s = config
                .getInitParameter(ApplicationConfig.JSR356_PATH_MAPPING_LENGTH);
        if (s != null) {
            pathLength = Integer.parseInt(s);
        }
        logger.trace("JSR356 Path mapping Size {}", pathLength);

        String servletPath = config
                .getInitParameter(ApplicationConfig.JSR356_MAPPING_PATH);
        if (servletPath == null) {
        ...
        configurator = IS_RUNNING_ON_QUARKUS
                ? new QuarkusAtmosphereConfigurator(config.framework())
        ...
        for (String endpointPath : endpointPaths) {
            if ("".equals(endpointPath)) {
                // Spec says: The path is always non-null and always begins with
                // a leading "/".
            ...
            try {
                ServerEndpointConfig endpointConfig = ServerEndpointConfig.Builder
                        .create(JSR356Endpoint.class, endpointPath)
                        .extensions(new ArrayList<>(
                                container.getInstalledExtensions()))
                        .configurator(configurator).build();
                container.addEndpoint(endpointConfig);
            } catch (DeploymentException e) {
                logger.warn(
                        "Duplicate Servlet Mapping Path {}. Use {} init-param to prevent this message",
    ...
    public final static class AtmosphereConfigurator
            extends ServerEndpointConfig.Configurator {

        private final AtmosphereFramework framework;
        /**
         * TODO: UGLY! GlassFish/Jetty call modifyHandshake BEFORE
         * getEndpointInstance() where other jsr356 do the reverse.
         */
        ...
        public void modifyHandshake(ServerEndpointConfig sec,
            ...
            // Broken GlassFish, which call modifyHandshake BEFORE
            // getEndpointInstance!
    ...
    static {
        ...
        try {
            ...
            // class not found, not running on Quarkus. Just ignore the
            // exception.
        }
        IS_RUNNING_ON_QUARKUS = runningOnQuarkus;
    }

    public final static class QuarkusAtmosphereConfigurator
            extends ServerEndpointConfig.Configurator {

        private final AtmosphereFramework framework;
        /**
         * TODO: UGLY! Quarkus call modifyHandshake BEFORE getEndpointInstance()
         * where other jsr356 do the reverse. However, because of VertX the
         * thread pool of Undertow handling regular http requests differ to the
         * thread pool handling websocket requests, so we can't use ThreadLocals
         * to remember the HandshakeRequest. Use BlockingDeque with the capacity
         * of 1 since we don't want the requests to get mixed up in case of high
         * concurrency.
         *
         * The reasoning for BlockingDeque is that we expect that a call to
         * getEndpointInstance() will immediately follow call to
         * modifyHandshake(). We will therefore store the request into this
         * deque, and we will pick it up in getEndpointInstance().
         *
         * The deque has a capacity of 1: if there are any concurrent attempts
         * to initialize the endpoint, any follow-up modifyHandshake() will be
         * delayed until the getEndpointInstance() is called.
         *
         * For the ThreadLocal-based solution see the original
         * JSR356AsyncSupport class from Atmosphere.
         */
        private final BlockingDeque<HandshakeRequest> requests = new LinkedBlockingDeque<>(
                1);
        ...
        public void modifyHandshake(ServerEndpointConfig sec,
                HandshakeRequest request, HandshakeResponse response) {
            try {
                // if there's an ongoing endpoint initialization, delay this
                // call
                // until the getEndpointInstance() has been called.
                if (!requests.offer(request, 1, TimeUnit.SECONDS)) {
                    throw new RuntimeException(
                            "Timed out waiting for getEndpointInstance() to be called");
                }
